...
{
    struct installAttri* att ;
    att = calloc(1, sizeof(struct installAttri )); 
    att = ( struct installAttri *) arg;
...
void installer(struct management_data *md) {
    ...
    for(int i=0; i<10;i++)
    {
        ...
        att = calloc(1, sizeof(struct installAttri )); 
        att->packageNum =i;
        att->md = calloc(1, sizeof(struct management_data )); 
        att->md = md;
        pthread_create(&md->tids[i], NULL, install,(void*) att);
